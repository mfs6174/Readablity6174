...
class PmlImage(Flowable, PmlMaxHeightMixIn):
    ...
    def getImage(self):
	#use appengine api to transform all image to jpg format
	#PIL which is needed to deal with non-jpg images is not pure python 
	#so can not run on appengine
	tmpimg=images.Image(self._imgdata)
	if tmpimg.format!=images.JPEG:
		tmpimg.resize(width=tmpimg.width, height=tmpimg.height)
		self._imgdata=tmpimg.execute_transforms(output_encoding=images.JPEG)
